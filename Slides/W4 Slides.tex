...
\begin{frame}{Concept Review from Assingment 1}

    \begin{enumerate}
        \item Efficiency: "the property of society getting the most it can from its scarce resources"
        \item Opportunity Cost: State What it is!!! State why the setting increases or decreases it!
        \item Marginal Analysis:
        \begin{itemize}
            \item What is the Marginal Benefit of X
            \item What is the Marginal Cost of X
            \item When do I do X? ($MB > MC$)
            \item When dynamics are involved, it's the standard of the field to talk about the static action ("doing laundry, not waiting to do laundry")
        \end{itemize}
        \item Trade: Beneficial for both countries (consuming outside of their PPF's) when exchange rate is between opportunity costs. However, why might it be reasonable to assume we don't see a lot of unfavorable trade?
    \end{enumerate}
...
\begin{frame}{Outline}
    ...
    \begin{block}{Elasticity is an intuitive concept: }
        How does an agent (consumer or producer) change their behavior in response to change of their circumstances (prices, incomes)?
    \end{block}

    \medskip

    We have different notions of elasticity. Today we will talk about \textit{price elasticity}. The same concept is extended to \textit{income elasticity} and \textit{cross-price elasticity} in your textbook. The idea is the same.
\end{frame}
...
\begin{frame}{Calculating elasticity}
    ...
    \medskip
    \[X\; \text{Elasticity of}\; Y = | \frac{\%\Delta Y}{\% \Delta X}|\]
